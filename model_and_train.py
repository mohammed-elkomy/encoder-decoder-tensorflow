...
with graph.as_default():
    ...
    with tf.variable_scope('train'):
        ...
        seqlen_mask = tf.transpose(tf.slice(tf.gather(lower_triangular_ones, decoder_inputs_length - 1), [0, 0], [batch_size_tf, tf.reduce_max(decoder_inputs_length)]))  # so you need to take length -1 due to lower triangle ones [sequence_length, batch_size]
        ...
        # To calculate the number correct, this means we don't count the padded
...
with tf.Session(graph=graph) as sess:
    ...
    for is_end_of_epoch, encoder_in, decoder_in, decoder_out, encoder_len, decoder_len in dataGenerator.next_batch(batch_size):
        ...
        if (current_time - summary_log_last) > summary_every_mins * minute and step != 0:
            ...
            training_summary_log(step, dataGenerator.current_epoch, tr_accuracy, loss_, t_perplexity, execution_start)
            ...
            for _ in range(10):
                ...
                # test_sequence_target = np.flip(test_sequence, axis=0)

                # for char in test_sequence:
                #     print(char[0], end=" ")
                # print("")
                #
                # for char in test_sequence_target:
                #     print(char[0], end=" ")
                # print("")
                ...
                for j in range(test_sequence.shape[0] + 5):
                ...
                cnt = 0
                # keep feeding the decoder,sample .. in a loop.. updating the step
                for i in range(test_sequence.shape[0] + 5):
                    feed_dict = {keep_prop_tf: 1.0,
                    ...
                    # feed tuple state
                    for j, v in enumerate(decoder_current_state):
                        feed_dict[v] = decoder_current_state_np[j]
                    ...
                    if i < test_sequence.shape[0] and (test_sequence_target[i][0] == sample - 1 or test_sequence_target[i][0] == sample == KOMYEOS):
                        cnt += 1
        ...
        if (current_time - checkpoint_last) > chechpoint_every_mins * minute:
            ...
            make_zip_results("NMT", step, outputFileName)
