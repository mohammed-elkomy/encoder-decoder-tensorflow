...
from model_graph import ModelGraphPureNoAtt
...
class TrainingManager(BaseTrainingManager):
    ...
    def local_machine_protection(self):  # my machine crashes due to memory limits
        ...
        self.configs.max_seq_len_encoder = 20
    ...
    def summary_callback(self, _next, graph: ModelGraphPureNoAtt, configs: ModelConfigs, sess: tf.Session):
    ...
    def sustaining_callback(self, _next, graph: ModelGraphPureNoAtt, configs: ModelConfigs, sess: tf.Session):
        encoder_in, decoder_in, decoder_out, encoder_decoder_len = _next
        feed_dict = {graph.keep_prop_tf: configs.keep_prop,
                     graph.encoder_inputs: encoder_in,
                     graph.encoder_inputs_length: encoder_decoder_len,
    ...
    def valid_example_callback(self, graph: ModelGraphPureNoAtt, configs: ModelConfigs, sess: tf.Session):
        ...
        for _ in range(5):
            ...
            decoder_current_state_np, = sess.run([graph.decoder_init_state_from_encoder], feed_dict=feed_dict)
    ...
    def end_epoch_callback(self, graph: ModelGraphPureNoAtt, configs: ModelConfigs, sess: tf.Session):
        self.logger.print_all("End of epoch ", self.global_step)
        pass

    def test_callback(self, graph: ModelGraphPureNoAtt, configs: ModelConfigs, sess: tf.Session):
        step = 0
