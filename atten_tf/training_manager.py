...
class TrainingManager(BaseTrainingManager):
    ...
    def summary_callback(self, _next, graph: ModelGraphPureAtt, configs: ModelConfigs, sess: tf.Session):
        ...
        self.logger.log_training_summary(self.acc_accuracy / self.step, self.acc_loss / self.step, self.acc_perplexity / self.step)

        # save training data for Tensorboard
        if not self.is_local_env:
            # noinspection PyUnboundLocalVariable
            self.training_writer.add_summary(summaries, self.global_step)

            summary_per = tf.Summary(value=[
                tf.Summary.Value(tag="perplexity", simple_value=t_perplexity),
            ])

            self.training_writer.add_summary(summary_per, self.global_step)

    def sustaining_callback(self, _next, graph: ModelGraphPureAtt, configs: ModelConfigs, sess: tf.Session):
        encoder_in, decoder_in, decoder_out, encoder_decoder_len = _next
        feed_dict = {graph.keep_prop_tf: configs.keep_prop,
                     graph.encoder_inputs: encoder_in,
                     graph.encoder_inputs_length: encoder_decoder_len,
    ...
    def valid_example_callback(self, graph: ModelGraphPureAtt, configs: ModelConfigs, sess: tf.Session):
        self.logger.print_all("Example generation")
        for _ in range(5):
            ...
            decoder_current_state_np, memory_fw_np, memory_bw_np = sess.run([graph.decoder_init_state_from_encoder, graph.memory_fw, graph.memory_bw], feed_dict=feed_dict)

            for char in test_sequence:
                if char == configs.named_constants.KOMYEOS:
                    self.logger.print_all("<KOMYEOS>", end=" ")
                else:
                    self.logger.print_all(char[0], end=" ")

            self.logger.print_all(" ==> ", end="")
            ...
            while j in range(test_sequence.shape[0] + 5) and next_feed[0, 0] != configs.named_constants.KOMYEOS:
                feed_dict = {graph.keep_prop_tf: 1.0,
                             ...
                             graph.memory_fw: memory_fw_np,  # memory forward outputs of encoder
                             graph.memory_bw: memory_bw_np  # memory backward outputs of encoder
                             }

                # feed tuple state
                for index, tensor_name in enumerate(graph.decoder_init_state_from_encoder):
                    feed_dict[tensor_name] = decoder_current_state_np[index]

                decoder_current_state_np, prediction_probabilities, _ = sess.run([graph.decoder_final_state, graph.dec_probabilities, graph.loop_outputs], feed_dict=feed_dict)  # wrap around the state and get the probabilities
                ...
                if next_feed[0, 0] == configs.named_constants.KOMYEOS:  # in decoder vocab
                    ...
                    self.logger.print_all("<KOMYSOS>", end=" ")
                elif next_feed[0, 0] == configs.named_constants.KOMYPAD:  # in decoder vocab
                    self.logger.print_all("<KOMYPAD>", end=" ")
                else:
                    self.logger.print_all(next_feed[0, 0] - 1, end=" ")  # in encoder vocab
            ...
            self.logger.print_all("({}/{})".format(cnt, test_sequence.shape[0]), ".")
            self.logger.print_all("*" * 50)

    def end_epoch_callback(self, graph: ModelGraphPureAtt, configs: ModelConfigs, sess: tf.Session):
        self.logger.print_all("End of epoch ", self.global_step)
        pass

    def test_callback(self, graph: ModelGraphPureAtt, configs: ModelConfigs, sess: tf.Session):
        step = 0
        test_data_generator = DataGenerator(self, "not train")
        ...
        for _, t_encoder_in, t_decoder_in, t_decoder_out, t_encoder_decoder_len in test_data_generator.next_batch():
            step += 1
            feed_dict = {graph.keep_prop_tf: 1.0,
            ...
            valid_loss += loss_
            neg_log_prop_ = negative_log_prob(prediction_probabilities, t_decoder_out)
            valid_logprob += neg_log_prop_

            if not self.is_local_env:
                self.validation_writer.add_summary(smm, self.global_step)

                self.validation_writer.add_summary(tf.Summary(value=[
                    tf.Summary.Value(tag="perplexity", simple_value=float(2 ** neg_log_prop_)),
                ]), self.global_step)
